...
class MMSpider:
    ...
    def __init__(self):
        ...
        # album有多少的的正则表达式
        self.__album_page_pattern = re.compile(
            '''<input name="totalPage" id="J_Totalpage" value="(.*?)"''', re.S)

    # 由页面返回的json数据得到字典,会得到30个模特的具体信息
    def get_person_dict(self, currentPage):
        try:
            data = urllib.parse.urlencode(
                {"currentPage": currentPage}).encode("utf-8")
    ...
    def get_album_ids(self, userId, page):
        try:
            all_album_url = self.__base_all_album_url + \
                str(userId) + "&" + str(page)
    ...
    def get_pic_page(self, userId, albumId):
        try:
            # 先得到这个相册一共有多少页
            url = self.__base_pic_url + \
                str(userId) + "&album_id=" + str(albumId)
